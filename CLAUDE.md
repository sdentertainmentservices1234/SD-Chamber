...
| `autofetch/fetch_causelist.py` + `autofetch/causelist-workflow.yml` | GitHub Action to auto-fetch/parse the daily SC cause list → commits `data/scindex-YYYY-MM-DD.json`. **NEVER RUN AGAINST THE LIVE SCI SITE** — see Pending. |
...
- `daysheets/{YYYY-MM-DD}`: {date, entries[], conferences[], updatedAt, updatedBy} —
  ...
  time (default "10.30"), bench, counsel (briefing counsel, autofilled from
  brief.aor), juniorUids[], juniorUid (legacy = juniorUids[0]; keep writing both,
  read via `jrsOf(e)`), remarks, done}. conferences: {time, name, briefId?} —
...
1. **Autofetch first live run** — `fetch_causelist.py` has never touched the
   real sci.gov.in. PDF-link discovery (`find_pdf_urls`) and parser regexes
   WILL need adjustment against real cause-list PDFs. The owner has a proven
   sibling pipeline (his ASD app parses api.sci.gov.in PDFs via Actions) —
   mine that repo's hardening if accessible.
2. Weight retuning + allocation feedback after real chamber use.
3. FCM push notifications via Firebase Functions (shared backend decision
