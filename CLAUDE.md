...
| `sw.js` | Minimal service worker (shell cache, network-first, never intercepts cross-origin). |
| `manifest.json` | PWA manifest. Icons (icon-192/512.png) not yet added. |
| `firestore.rules` | Security rules (reconstructed Jul 2026) — must match app writes; diff against live console before publishing. |
...
- `manifest.json` references icon-192/512.png which don't exist yet (install
  prompt works but iconless). A chamber monogram would finish it.
